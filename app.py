...
def create_docx_logic(text_content, branding_info, sow_type_name):
    """
    Generates the Word document with strict page flow.
    Page 1: Cover
    Page 2: TOC Only
    Page 3: Project Overview onwards
    """
    ...
    # We'll parse the content and find Section 1
    ...
    while i < len(lines):
        ...
        # Handle Section 1: TOC
        if "1 TABLE OF CONTENTS" in upper_text and not toc_added:
            doc.add_heading("1 TABLE OF CONTENTS", level=1)
            # Find subsequent TOC items until Section 2
            i += 1
            while i < len(lines):
                ...
                if not next_line:
                    i += 1
                    continue
                if "2 PROJECT OVERVIEW" in next_line.upper():
                    break
                # Process TOC lines
                nl_clean = re.sub(r'\*+', '', next_line).strip()
                nl_text = re.sub(r'^#+\s*', '', nl_clean).strip()
                p = doc.add_paragraph(nl_text)
                p.paragraph_format.left_indent = Inches(0.4) if (nl_text[0].isdigit() and "." in nl_text) else Pt(0)
                i += 1
        ...
        # Handle Section 2: Project Overview (Page 3)
        ...
        # Table processing with hyperlink mapping
                        ...
                        for idx, c_text in enumerate(r_data):
                            if idx < len(row_cells):
                                if "Estimate" in c_text:
                                    ...
                                    # Specific mapping for individual links
                                    calc_url = CALCULATOR_LINKS.get(sow_type_name, "https://calculator.aws")
                                    # Beauty Advisor conditional logic
        ...
        if line.startswith('# '):
            ...
            # Bold keywords for stakeholders
            segregation_keywords = ["PARTNER EXECUTIVE SPONSOR", "CUSTOMER EXECUTIVE SPONSOR", "AWS EXECUTIVE SPONSOR", "PROJECT ESCALATION CONTACTS", "DEPENDENCIES:", "ASSUMPTIONS:"]
...
if st.button("✨ Generate SOW Document", type="primary", use_container_width=True):
    if not api_key:
        ...
        with st.spinner(f"Architecting {selected_sow_name}..."):
            ...
            1 TABLE OF CONTENTS
            2 PROJECT OVERVIEW
              2.1 OBJECTIVE (Strictly 2-3 lines based on input: {objective})
            ...
            Provide a technical project plan with clear phases.
            ...
            payload = {
                ...
                "systemInstruction": {"parts": [{"text": "You are a senior Solutions Architect. Ensure Section 5 is included precisely as requested. Page 1 is cover, Page 2 is TOC only, Page 3 is Overview. No markdown bolding."}]}
            }
...
if st.session_state.generated_sow:
    ...
    with tab_preview:
        ...
        preview_c = st.session_state.generated_sow.replace("Estimate", f'<a href="{calc_url_p}" target="_blank" style="color:#3b82f6; text-decoration: underline;">Estimate</a>')
        
        header_pattern = r'(?i)(^#*\s*4\s+SOLUTION ARCHITECTURE.*)'
        match = re.search(header_pattern, preview_c, re.MULTILINE)
        
        if match:
            start, end = match.span()
            st.markdown(preview_c[:end], unsafe_allow_html=True)
            ...
            st.markdown(preview_c, unsafe_allow_html=True)
